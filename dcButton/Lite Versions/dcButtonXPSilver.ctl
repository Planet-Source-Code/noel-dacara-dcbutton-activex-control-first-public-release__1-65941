VERSION 5.00
Begin VB.UserControl dcButtonXPSilver 
   AutoRedraw      =   -1  'True
   ClientHeight    =   495
   ClientLeft      =   0
   ClientTop       =   0
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   100
   ToolboxBitmap   =   "dcButtonXPSilver.ctx":0000
End
Attribute VB_Name = "dcButtonXPSilver"
...
'
'           Don't worry about the size of this file, about 10-20% of these codes
...
Public Enum eXPSilverButtonShapes
    evsCutNone  ' Normal
    evsCutLeft  '
    evsCutRight '
    evsCutSides ' Both left & right
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private evsCutNone, evsCutLeft, evsCutRight, evsCutSides
#End If

Private Enum eXPSilverButtonStates
    evsNormal   ' Normal state
    evsHot      ' Cursor is over the control
    evsDown     ' Mouse/key is down
    evsDisabled ' Disabled state
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private evsNormal, evsHot, evsDown, evsDisabled
#End If

Public Enum eXPSilverPicAlignments
    evaBehindText
    evaBottomEdge
    evaBottomOfCaption
    evaLeftEdge
...
#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private evaBehindText, evaBottomEdge, evaBottomOfCaption
    Private evaLeftEdge, evaLeftOfCaption, evaRightEdge
    Private evaRightOfCaption, evaTopEdge, evaTopOfCaption
#End If

Public Enum eXPSilverPictureSizes
    evpNormal ' Use original size of main picture
    evp16x16  ' Small icon size
    evp24x24  ' Standard toolbar icon size
    evp32x32  ' Standard icon size
    evp48x48  ' Explorer thumbnail size
    evpCustom ' Use picture size defined in property
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private evpNormal, evp16x16, evp24x24, evp32x32, evp48x48, evpCustom
#End If

Public Enum eXPSilverUserColors ' User-defined colors
    evcDownColor    ' ---
    evcFocusBorder  ' Available for use with the OverrideColor procedure
    evcGrayColor    ' For advance users only
    evcGrayText     ' Not accessible using VB IDE's properties window
    evcHoverColor   ' Note: Color property usage may vary between button styles
    evcStartColor   ' ---
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private evcDownColor, evcFocusBorder, evcGrayColor
    Private evcGrayText, evcHoverColor, evcStartColor
#End If
...
Private Type tButtonProperties ' Cached button properties
    ...
    MaskColor   As Long
    PicAlign    As eXPSilverPicAlignments
    PicDown     As StdPicture
    PicHot      As StdPicture
    PicNormal   As StdPicture
    PicOpacity  As Single
    PicSize     As eXPSilverPictureSizes
    PicSizeH    As Long
    PicSizeW    As Long
    Shape       As eXPSilverButtonShapes
    UseMask     As Boolean
    Value       As Boolean
End Type
...
Private Type tButtonSettings        ' Runtime/designtime settings
    ...
    State       As eXPSilverButtonStates    ' Current drawing state of the button
...
Public Property Get ButtonShape() As eXPSilverButtonShapes
...
Public Property Let ButtonShape(Value As eXPSilverButtonShapes)
...
Public Property Get PictureAlignment() As eXPSilverPicAlignments
...
Public Property Let PictureAlignment(Value As eXPSilverPicAlignments)
...
'   Else the specified value will be set as the main picture instead.
    ...
    If (m_tButtonProperty.PicNormal Is Nothing) Then
        ...
        Exit Property
    Else
        If (m_tButtonProperty.PicSize = evpNormal) Then
...
Public Property Get PictureSize() As eXPSilverPictureSizes
...
Public Property Let PictureSize(Value As eXPSilverPictureSizes)
    If (m_tButtonProperty.PicNormal Is Nothing) Then
        m_tButtonProperty.PicSize = evpNormal
...
Public Property Let PictureSizeH(Value As Long)
    m_tButtonProperty.PicSize = evpCustom   ' If modified then set size to custom
...
Public Sub OverrideColor( _
        Property As eXPSilverUserColors, _
...
'   Forces a complete revaint of the button.
...
'   this procedure is called only when the control has encountered a decisive change.
    ...
    Select Case m_tButtonProperty.PicAlign                          ' Picture on center
        Case evaBehindText                                          ' but behind text
            OffsetRect pr, (bw - pr.Right) \ 2, (bh - pr.Bottom) \ 2
                                                                    '
        Case evaBottomEdge, evaBottomOfCaption                      ' Picture on
            OffsetRect pr, (bw - pr.Right) \ 2, 0                   ' bottom portion
            OffsetRect tr, 0, -tr.Top                               ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = evaBottomEdge) Then    '
            ...
            End If                                                  '
                                                                    '
        Case evaLeftEdge, evaLeftOfCaption                          ' Picture on
            OffsetRect pr, 0, (bh - pr.Bottom) \ 2                  ' left portion
            OffsetRect tr, -tr.Left, 0                              ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = evaLeftEdge) Then      '
            ...
            End If                                                  '
                                                                    '
        Case evaRightEdge, evaRightOfCaption                        ' Picture on
            OffsetRect pr, 0, (bh - pr.Bottom) \ 2                  ' right portion
            OffsetRect tr, -tr.Left, 0                              ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = evaRightEdge) Then     '
            ...
            End If                                                  '
                                                                    '
        Case evaTopEdge, evaTopOfCaption                            ' Picture on
            OffsetRect pr, (bw - pr.Right) \ 2, 0                   ' top portion
            OffsetRect tr, 0, -tr.Top                               ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = evaTopEdge) Then       '
...
'   Create a button region with rounded corners.
    ...
    With m_tButtonSettings
        ...
        If (m_tButtonProperty.Shape = evsCutLeft) Or _
           (m_tButtonProperty.Shape = evsCutSides) Then
            hRgn2 = CreateRectRgn(0, 0, .Width / 2, .Height + 1)
            CombineRgn hRgn, hRgn, hRgn2, RGN_OR
            DeleteObject hRgn2
        End If
        
        If (m_tButtonProperty.Shape = evsCutRight) Or _
           (m_tButtonProperty.Shape = evsCutSides) Then
...
Private Sub DrawButton(Optional State As eXPSilverButtonStates = -1, Optional Force As Boolean)
'   Draws the button itself
    ...
    DrawButtonXPStyle State
    
End Sub

Private Sub DrawButtonXPStyle(DrawState As eXPSilverButtonStates)
'   Drawing procedure for the xp button styles.
    ...
    With m_tButtonProperty
        If (.CheckBox And .Value And .Enabled) Then ' Checkbox on checked state
            ...
            If (.Style = evsXPSilver) Then          '
                DrawState = evsDown                 '
            #If USE_XPBLUE Or USE_XPOLIVEGREEN Then ' There are some codes that
            ElseIf (DrawState = evsDown) Then       ' are seemed to be repeated
                DrawState = evsNormal               ' but they are intended to
            #End If                                 ' be arranged in that way...
            End If                                  '
            #Else                                   ' You don't need to fully
                If (DrawState = evsDown) Then       ' understand why coz' it really
                    DrawState = evsNormal           ' doesn't matter, they are just
    ...
    With m_tButtonSettings
        If (.State = evsDisabled) Then ' Disabled state
        ...
        Else
            ...
            If (.State = evsHot) Or (m_bMouseIsDown And Not m_bMouseOnButton And Not DrawState = evsDown) Then
                ' Hot state border (not shown when button is on checked state)
                If (Not m_tButtonProperty.Value) Then
                    Color1 = BlendColors(m_tButtonColors.HoverColor, &HFFFFFF)                  ' Prevare colors
            ...
            Else
                ...
                If (m_bParentActive) And (m_bButtonHasFocus And Not .State = evsDown And Not m_tButtonProperty.Value) Then
                    RectangleEx m_tButtonColors.StartColor, 2, 1, .Width - 2, .Height
                Else
                    RectangleEx m_tButtonColors.StartColor, 1, 1, .Width - 1, .Height
                End If
                
                If (m_bParentActive) And (Not .State = evsDown And (m_bButtonHasFocus Or .Default)) Then
                    ' Focus state border (not shown when button is in checked state)
                    If (Not m_tButtonProperty.Value) Then
                        Color1 = BlendColors(m_tButtonColors.FocusBorder, &HFFFFFF, 0.4)            ' Prevare colors
        ...
        If Not (m_tButtonProperty.Shape = evsCutLeft Or _
                m_tButtonProperty.Shape = evsCutSides) Then
            SetPixelV hDC, 1, 1, Color1
            SetPixelV hDC, 1, .Height - 2, Color1
        End If
        
        If Not (m_tButtonProperty.Shape = evsCutRight Or _
                m_tButtonProperty.Shape = evsCutSides) Then
...
'   Draw vertical gradient effect on the control on specified coordinates.
    ...
    While (Y1 <= Y2)                            ' Draw from top to bottom
        ...
        End If                                  '
                                                '
        Color = RGBEx(RGB4.R, RGB4.G, RGB4.B)   ' Prevare color
                                                '
        If (X1 = X2) Then                       '
...
'   Draw icon to button with the specified settings.
    ...
    If (m_tButtonProperty.PicSize = evsNormal) Then
    ...
    If (m_tButtonSettings.State = evsDisabled) Then ' For disabled buttons
...
Jmp_DrawImage:
    ...
    If (m_tButtonSettings.State = evsDisabled) Then '
...
'   Draw icon to button with defined special effect being applied.
    ...
    If (m_tButtonSettings.State = evsHot And Not m_tButtonProperty.PicHot Is Nothing) Then
        Set Picture = m_tButtonProperty.PicHot
    ElseIf (m_tButtonSettings.State = evsDown) Or (m_tButtonProperty.Value) Then
        ...
        Case WM_MOUSELEAVE
            ...
            If (Not m_bSpacebarIsDown) Then
                If (m_tButtonProperty.Enabled) Then
                    DrawButton evsNormal, (m_tButtonSettings.Button = 1)
                End If
...
'   Triggered normally when user clicks the control and release it inside the button
    ...
    With m_tButtonProperty                  '
        ...
        If (Not m_bMouseOnButton) Then      ' Check if cursor is over the control
            DrawButton evsNormal, True      ' Redraw is necessary if it is not :)
        Else                                '
            DrawButton evsHot, .CheckBox    ' Force redraw for checkbox mode
        End If                              '
                                            '
        RaiseEvent Click                    '
                                            '
...
'   Only raised when another control of the same window is focused
    ...
    If (m_tButtonProperty.Enabled) Then '
        If (m_bParentActive) Then       ' We need to force redraw
            DrawButton evsNormal, True  ' button on lost of control focus
...
Private Sub UserControl_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If (Button = 1) Then                    ' vbLeftButton
        ...
        ElseIf (Not m_bMouseOnButton) Then  ' If mouse button is pressed outside
            ...
            DrawButton evsNormal            ' being held down then draw hot state
    ...
    Else
        ...
        If (Not m_bSpacebarIsDown) Then ' Check if spacebar is held down
            ...
            Else                        ' If button should be in down state then
                DrawButton evsHot       ' draw the down state else draw the hot state
            End If                      '
        ElseIf (m_bMouseIsDown) Then    ' Else, check if mouse is held down
            DrawButton evsDown          ' as it moves over the button to
...
'   (On loading of form and before the is control is unloaded to design mode)
    ...
    With PropBag
        ...
        m_tButtonProperty.PicAlign = .ReadProperty("PicAlign", evaLeftOfCaption)
    ...
    Set m_tButtonProperty.PicNormal = .ReadProperty("PicNormal", Nothing)
        ...
        m_tButtonProperty.PicSize = .ReadProperty("PicSize", 0) ' evpNormal
        m_tButtonProperty.PicSizeH = .ReadProperty("PicSizeH", 0)
        m_tButtonProperty.PicSizeW = .ReadProperty("PicSizeW", 0)
        m_tButtonSettings.State = .ReadProperty("State", 0) ' evsNormal
...
'   but only when there are changes in the properties to be updated
    ...
    With PropBag
        ...
        .WriteProperty "PicAlign", m_tButtonProperty.PicAlign, evaLeftOfCaption
        ...
        .WriteProperty "PicSize", m_tButtonProperty.PicSize, 0 ' evpNormal
        .WriteProperty "PicSizeH", m_tButtonProperty.PicSizeH, 0
        .WriteProperty "PicSizeW", m_tButtonProperty.PicSizeW, 0
        .WriteProperty "State", m_tButtonSettings.State, 0 ' evsNormal
