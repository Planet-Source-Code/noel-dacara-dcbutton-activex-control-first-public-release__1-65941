VERSION 5.00
Begin VB.UserControl dcButtonCrystal 
   AutoRedraw      =   -1  'True
   ClientHeight    =   495
   ClientLeft      =   0
   ClientTop       =   0
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   100
   ToolboxBitmap   =   "dcButtonCrystal.ctx":0000
End
Attribute VB_Name = "dcButtonCrystal"
...
Public Enum eCrystalButtonShapes
    ecsCutNone  ' Normal
    ecsCutLeft  '
    ecsCutRight '
    ecsCutSides ' Both left & right
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private ecsCutNone, ecsCutLeft, ecsCutRight, ecsCutSides
#End If

Private Enum eCrystalButtonStates
    ecsNormal   ' Normal state
    ecsHot      ' Cursor is over the control
    ecsDown     ' Mouse/key is down
    ecsDisabled ' Disabled state
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private ecsNormal, ecsHot, ecsDown, ecsDisabled
#End If

Public Enum eCrystalPicAlignments
    ecaBehindText
    ecaBottomEdge
    ecaBottomOfCaption
    ecaLeftEdge
...
#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private ecaBehindText, ecaBottomEdge, ecaBottomOfCaption
    Private ecaLeftEdge, ecaLeftOfCaption, ecaRightEdge
    Private ecaRightOfCaption, ecaTopEdge, ecaTopOfCaption
#End If

Public Enum eCrystalPictureSizes
    ecpNormal ' Use original size of main picture
    ecp16x16  ' Small icon size
    ecp24x24  ' Standard toolbar icon size
    ecp32x32  ' Standard icon size
    ecp48x48  ' Explorer thumbnail size
    ecpCustom ' Use picture size defined in property
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private ecpNormal, ecp16x16, ecp24x24, ecp32x32, ecp48x48, ecpCustom
#End If

Public Enum eCrystalUserColors ' User-defined colors
    eccDownColor    ' ---
    eccFocusBorder  ' Available for use with the OverrideColor procedure
    eccGrayColor    ' For advance users only
    eccGrayText     ' Not accessible using VB IDE's properties window
    eccHoverColor   ' Note: Color property usage may vary between button styles
    eccStartColor   ' ---
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private eccDownColor, eccFocusBorder, eccGrayColor
    Private eccGrayText, eccHoverColor, eccStartColor
#End If
...
Private Type tButtonProperties ' Cached button properties
    ...
    MaskColor   As Long
    PicAlign    As eCrystalPicAlignments
    PicDown     As StdPicture
    PicHot      As StdPicture
    PicNormal   As StdPicture
    PicOpacity  As Single
    PicSize     As eCrystalPictureSizes
    PicSizeH    As Long
    PicSizeW    As Long
    Shape       As eCrystalButtonShapes
    UseMask     As Boolean
    Value       As Boolean
End Type
...
Private Type tButtonSettings        ' Runtime/designtime settings
    ...
    State       As eCrystalButtonStates    ' Current drawing state of the button
...
Public Property Get ButtonShape() As eCrystalButtonShapes
...
Public Property Let ButtonShape(Value As eCrystalButtonShapes)
...
Public Property Get PictureAlignment() As eCrystalPicAlignments
...
Public Property Let PictureAlignment(Value As eCrystalPicAlignments)
...
'   Else the specified value will be set as the main picture instead.
    ...
    If (m_tButtonProperty.PicNormal Is Nothing) Then
        ...
        Exit Property
    Else
        If (m_tButtonProperty.PicSize = ecpNormal) Then
...
Public Property Get PictureSize() As eCrystalPictureSizes
...
Public Property Let PictureSize(Value As eCrystalPictureSizes)
    If (m_tButtonProperty.PicNormal Is Nothing) Then
        m_tButtonProperty.PicSize = ecpNormal
...
Public Property Let PictureSizeH(Value As Long)
    m_tButtonProperty.PicSize = ecpCustom   ' If modified then set size to custom
...
Public Sub OverrideColor( _
        Property As eCrystalUserColors, _
...
'   this procedure is called only when the control has encountered a decisive change.
    ...
    Select Case m_tButtonProperty.PicAlign                          ' Picture on center
        Case ecaBehindText                                          ' but behind text
            OffsetRect pr, (bw - pr.Right) \ 2, (bh - pr.Bottom) \ 2
                                                                    '
        Case ecaBottomEdge, ecaBottomOfCaption                      ' Picture on
            OffsetRect pr, (bw - pr.Right) \ 2, 0                   ' bottom portion
            OffsetRect tr, 0, -tr.Top                               ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = ecaBottomEdge) Then    '
            ...
            End If                                                  '
                                                                    '
        Case ecaLeftEdge, ecaLeftOfCaption                          ' Picture on
            OffsetRect pr, 0, (bh - pr.Bottom) \ 2                  ' left portion
            OffsetRect tr, -tr.Left, 0                              ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = ecaLeftEdge) Then      '
            ...
            End If                                                  '
                                                                    '
        Case ecaRightEdge, ecaRightOfCaption                        ' Picture on
            OffsetRect pr, 0, (bh - pr.Bottom) \ 2                  ' right portion
            OffsetRect tr, -tr.Left, 0                              ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = ecaRightEdge) Then     '
            ...
            End If                                                  '
                                                                    '
        Case ecaTopEdge, ecaTopOfCaption                            ' Picture on
            OffsetRect pr, (bw - pr.Right) \ 2, 0                   ' top portion
            OffsetRect tr, 0, -tr.Top                               ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = ecaTopEdge) Then       '
...
'   Calculate points needed to draw border of round-rectangular regions.
    ...
    If (m_tButtonProperty.Shape = ecsCutLeft) Or _
       (m_tButtonProperty.Shape = ecsCutSides) Then
    ...
    If (m_tButtonProperty.Shape = ecsCutRight) Or _
       (m_tButtonProperty.Shape = ecsCutSides) Then
...
Private Sub DrawButton(Optional State As eCrystalButtonStates = -1, Optional Force As Boolean)
'   Draws the button itself
    ...
    DrawButtonCrystalMac State
    
End Sub

Private Sub DrawButtonCrystalMac(DrawState As eCrystalButtonStates)
'   Drawing procedure for Crystal.
    ...
    With m_tButtonProperty
        If (.CheckBox And .Value And .Enabled) Then ' Button on checked state
            Select Case DrawState
                Case ecsNormal:     DrawState = ecsDown
                Case ecsHot:        DrawState = ecsDown
                Case ecsDown:       DrawState = ecsDown
            End Select
        ElseIf (DrawState = ecsNormal) And (m_bMouseIsDown) Then
                DrawState = ecsHot
        End If
    End With
    ...
    With m_tButtonColors
        Select Case DrawState ' Set background color
            Case ecsNormal:     Color1 = .BackColor
            Case ecsHot:        Color1 = .HoverColor
            Case ecsDown:       Color1 = .DownColor
            Case ecsDisabled:   Color1 = .GrayColor
        End Select
    ...
    With m_tButtonSettings
        If (.State = ecsDisabled) Then ' Disabled state
            ...
            End If
        Else
            If (.State = ecsDown) Or (m_tButtonProperty.Value) Then
                If (m_tButtonProperty.Value) And (.State = ecsDown) Then
...
'   Draw icon to button with the specified settings.
    ...
    If (m_tButtonProperty.PicSize = ecsNormal) Then
    ...
    If (m_tButtonSettings.State = ecsDisabled) Then ' For disabled buttons
...
Jmp_DrawImage:
    ...
    If (m_tButtonSettings.State = ecsDisabled) Then '
...
'   Draw icon to button with defined special effect being applied.
    ...
    If (m_tButtonSettings.State = ecsHot And Not m_tButtonProperty.PicHot Is Nothing) Then
        Set Picture = m_tButtonProperty.PicHot
    ElseIf (m_tButtonSettings.State = ecsDown) Or (m_tButtonProperty.Value) Then
        ...
        Case WM_MOUSELEAVE
            ...
            If (Not m_bSpacebarIsDown) Then
                If (m_tButtonProperty.Enabled) Then
                    DrawButton ecsNormal, (m_tButtonSettings.Button = 1)
                End If
...
'   Triggered normally when user clicks the control and release it inside the button
    ...
    With m_tButtonProperty                  '
        ...
        If (Not m_bMouseOnButton) Then      ' Check if cursor is over the control
            DrawButton ecsNormal, True      ' Redraw is necessary if it is not :)
        Else                                '
            DrawButton ecsHot, .CheckBox    ' Force redraw for checkbox mode
        End If                              '
                                            '
        RaiseEvent Click                    '
                                            '
...
'   Only raised when another control of the same window is focused
    ...
    If (m_tButtonProperty.Enabled) Then '
        If (m_bParentActive) Then       ' We need to force redraw
            DrawButton ecsNormal, True  ' button on lost of control focus
...
Private Sub UserControl_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If (Button = 1) Then                    ' vbLeftButton
        ...
        ElseIf (Not m_bMouseOnButton) Then  ' If mouse button is pressed outside
            ...
            DrawButton ecsNormal            ' being held down then draw hot state
    ...
    Else
        ...
        If (Not m_bSpacebarIsDown) Then ' Check if spacebar is held down
            ...
            Else                        ' If button should be in down state then
                DrawButton ecsHot       ' draw the down state else draw the hot state
            End If                      '
        ElseIf (m_bMouseIsDown) Then    ' Else, check if mouse is held down
            DrawButton ecsDown          ' as it moves over the button to
...
'   (On loading of form and before the is control is unloaded to design mode)
    ...
    With PropBag
        ...
        m_tButtonProperty.PicAlign = .ReadProperty("PicAlign", ecaLeftOfCaption)
    ...
    Set m_tButtonProperty.PicNormal = .ReadProperty("PicNormal", Nothing)
        ...
        m_tButtonProperty.PicSize = .ReadProperty("PicSize", 0) ' ecpNormal
        m_tButtonProperty.PicSizeH = .ReadProperty("PicSizeH", 0)
        m_tButtonProperty.PicSizeW = .ReadProperty("PicSizeW", 0)
        m_tButtonSettings.State = .ReadProperty("State", 0) ' ecsNormal
...
'   but only when there are changes in the properties to be updated
    ...
    With PropBag
        ...
        .WriteProperty "PicAlign", m_tButtonProperty.PicAlign, ecaLeftOfCaption
        ...
        .WriteProperty "PicSize", m_tButtonProperty.PicSize, 0 ' ecpNormal
        .WriteProperty "PicSizeH", m_tButtonProperty.PicSizeH, 0
        .WriteProperty "PicSizeW", m_tButtonProperty.PicSizeW, 0
        .WriteProperty "State", m_tButtonSettings.State, 0 ' ecsNormal
