VERSION 5.00
Begin VB.UserControl dcButtonMacOSx 
   AutoRedraw      =   -1  'True
   ClientHeight    =   495
   ClientLeft      =   0
   ClientTop       =   0
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   100
   ToolboxBitmap   =   "dcButtonMacOSx.ctx":0000
End
Attribute VB_Name = "dcButtonMacOSx"
...
Public Enum eMacOSxButtonShapes
    exsCutNone  ' Normal
    exsCutLeft  '
    exsCutRight '
    exsCutSides ' Both left & right
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private exsCutNone, exsCutLeft, exsCutRight, exsCutSides
#End If

Private Enum eMacOSxButtonStates
    exsNormal   ' Normal state
    exsHot      ' Cursor is over the control
    exsDown     ' Mouse/key is down
    exsDisabled ' Disabled state
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private exsNormal, exsHot, exsDown, exsDisabled
#End If

Public Enum eMacOSxPicAlignments
    exaBehindText
    exaBottomEdge
    exaBottomOfCaption
    exaLeftEdge
...
#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private exaBehindText, exaBottomEdge, exaBottomOfCaption
    Private exaLeftEdge, exaLeftOfCaption, exaRightEdge
    Private exaRightOfCaption, exaTopEdge, exaTopOfCaption
#End If

Public Enum eMacOSxPictureSizes
    expNormal ' Use original size of main picture
    exp16x16  ' Small icon size
    exp24x24  ' Standard toolbar icon size
    exp32x32  ' Standard icon size
    exp48x48  ' Explorer thumbnail size
    expCustom ' Use picture size defined in property
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private expNormal, exp16x16, exp24x24, exp32x32, exp48x48, expCustom
#End If

Public Enum eMacOSxUserColors ' User-defined colors
    excDownColor    ' ---
    excFocusBorder  ' Available for use with the OverrideColor procedure
    excGrayColor    ' For advance users only
    excGrayText     ' Not accessible using VB IDE's properties window
    excHoverColor   ' Note: Color property usage may vary between button styles
    excStartColor   ' ---
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private excDownColor, excFocusBorder, excGrayColor
    Private excGrayText, excHoverColor, excStartColor
#End If
...
Private Type tButtonProperties ' Cached button properties
    ...
    MaskColor   As Long
    PicAlign    As eMacOSxPicAlignments
    PicDown     As StdPicture
    PicHot      As StdPicture
    PicNormal   As StdPicture
    PicOpacity  As Single
    PicSize     As eMacOSxPictureSizes
    PicSizeH    As Long
    PicSizeW    As Long
    Shape       As eMacOSxButtonShapes
    UseMask     As Boolean
    Value       As Boolean
End Type
...
Private Type tButtonSettings        ' Runtime/designtime settings
    ...
    State       As eMacOSxButtonStates    ' Current drawing state of the button
...
Public Property Get ButtonShape() As eMacOSxButtonShapes
...
Public Property Let ButtonShape(Value As eMacOSxButtonShapes)
...
Public Property Get PictureAlignment() As eMacOSxPicAlignments
...
Public Property Let PictureAlignment(Value As eMacOSxPicAlignments)
...
'   Else the specified value will be set as the main picture instead.
    ...
    If (m_tButtonProperty.PicNormal Is Nothing) Then
        ...
        Exit Property
    Else
        If (m_tButtonProperty.PicSize = expNormal) Then
...
Public Property Get PictureSize() As eMacOSxPictureSizes
...
Public Property Let PictureSize(Value As eMacOSxPictureSizes)
    If (m_tButtonProperty.PicNormal Is Nothing) Then
        m_tButtonProperty.PicSize = expNormal
...
Public Property Let PictureSizeH(Value As Long)
    m_tButtonProperty.PicSize = expCustom   ' If modified then set size to custom
...
Public Sub OverrideColor( _
        Property As eMacOSxUserColors, _
...
'   this procedure is called only when the control has encountered a decisive change.
    ...
    Select Case m_tButtonProperty.PicAlign                          ' Picture on center
        Case exaBehindText                                          ' but behind text
            OffsetRect pr, (bw - pr.Right) \ 2, (bh - pr.Bottom) \ 2
                                                                    '
        Case exaBottomEdge, exaBottomOfCaption                      ' Picture on
            OffsetRect pr, (bw - pr.Right) \ 2, 0                   ' bottom portion
            OffsetRect tr, 0, -tr.Top                               ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = exaBottomEdge) Then    '
            ...
            End If                                                  '
                                                                    '
        Case exaLeftEdge, exaLeftOfCaption                          ' Picture on
            OffsetRect pr, 0, (bh - pr.Bottom) \ 2                  ' left portion
            OffsetRect tr, -tr.Left, 0                              ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = exaLeftEdge) Then      '
            ...
            End If                                                  '
                                                                    '
        Case exaRightEdge, exaRightOfCaption                        ' Picture on
            OffsetRect pr, 0, (bh - pr.Bottom) \ 2                  ' right portion
            OffsetRect tr, -tr.Left, 0                              ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = exaRightEdge) Then     '
            ...
            End If                                                  '
                                                                    '
        Case exaTopEdge, exaTopOfCaption                            ' Picture on
            OffsetRect pr, (bw - pr.Right) \ 2, 0                   ' top portion
            OffsetRect tr, 0, -tr.Top                               ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = exaTopEdge) Then       '
...
'   Calculate points needed to draw border of round-rectangular regions.
    ...
    If (m_tButtonProperty.Shape = exsCutLeft) Or _
       (m_tButtonProperty.Shape = exsCutSides) Then
    ...
    If (m_tButtonProperty.Shape = exsCutRight) Or _
       (m_tButtonProperty.Shape = exsCutSides) Then
...
Private Sub DrawButton(Optional State As eMacOSxButtonStates = -1, Optional Force As Boolean)
...
Private Sub DrawButtonCrystalMac(DrawState As eMacOSxButtonStates)
'   Drawing procedure for Crystal.
    ...
    With m_tButtonProperty
        If (.CheckBox And .Value And .Enabled) Then ' Button on checked state
            Select Case DrawState
                Case exsNormal:     DrawState = exsDown
                Case exsHot:        DrawState = exsDown
                Case exsDown:       DrawState = exsDown
            End Select
        ElseIf (DrawState = exsNormal) And (m_bMouseIsDown) Then
                DrawState = exsHot
        End If
    End With
    ...
    With m_tButtonColors
        Select Case DrawState ' Set background color
            Case exsNormal:     Color1 = .BackColor
            Case exsHot:        Color1 = .HoverColor
            Case exsDown:       Color1 = .DownColor
            Case exsDisabled:   Color1 = .GrayColor
        End Select
    ...
    With m_tButtonSettings
        If (.State = exsDisabled) Then ' Disabled state
            ...
            End If
        Else
            If (.State = exsDown) Or (m_tButtonProperty.Value) Then
                If (m_tButtonProperty.Value) And (.State = exsDown) Then
...
'   Draw icon to button with the specified settings.
    ...
    If (m_tButtonProperty.PicSize = exsNormal) Then
    ...
    If (m_tButtonSettings.State = exsDisabled) Then ' For disabled buttons
...
Jmp_DrawImage:
    ...
    If (m_tButtonSettings.State = exsDisabled) Then '
...
'   Draw icon to button with defined special effect being applied.
    ...
    If (m_tButtonSettings.State = exsHot And Not m_tButtonProperty.PicHot Is Nothing) Then
        Set Picture = m_tButtonProperty.PicHot
    ElseIf (m_tButtonSettings.State = exsDown) Or (m_tButtonProperty.Value) Then
        ...
        Case WM_MOUSELEAVE
            ...
            If (Not m_bSpacebarIsDown) Then
                If (m_tButtonProperty.Enabled) Then
                    DrawButton exsNormal, (m_tButtonSettings.Button = 1)
                End If
...
'   Triggered normally when user clicks the control and release it inside the button
    ...
    With m_tButtonProperty                  '
        ...
        If (Not m_bMouseOnButton) Then      ' Check if cursor is over the control
            DrawButton exsNormal, True      ' Redraw is necessary if it is not :)
        Else                                '
            DrawButton exsHot, .CheckBox    ' Force redraw for checkbox mode
        End If                              '
                                            '
        RaiseEvent Click                    '
                                            '
...
'   Only raised when another control of the same window is focused
    ...
    If (m_tButtonProperty.Enabled) Then '
        If (m_bParentActive) Then       ' We need to force redraw
            DrawButton exsNormal, True  ' button on lost of control focus
...
Private Sub UserControl_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If (Button = 1) Then                    ' vbLeftButton
        ...
        ElseIf (Not m_bMouseOnButton) Then  ' If mouse button is pressed outside
            ...
            DrawButton exsNormal            ' being held down then draw hot state
    ...
    Else
        ...
        If (Not m_bSpacebarIsDown) Then ' Check if spacebar is held down
            ...
            Else                        ' If button should be in down state then
                DrawButton exsHot       ' draw the down state else draw the hot state
            End If                      '
        ElseIf (m_bMouseIsDown) Then    ' Else, check if mouse is held down
            DrawButton exsDown          ' as it moves over the button to
...
'   (On loading of form and before the is control is unloaded to design mode)
    ...
    With PropBag
        ...
        m_tButtonProperty.PicAlign = .ReadProperty("PicAlign", exaLeftOfCaption)
    ...
    Set m_tButtonProperty.PicNormal = .ReadProperty("PicNormal", Nothing)
        ...
        m_tButtonProperty.PicSize = .ReadProperty("PicSize", 0) ' expNormal
        m_tButtonProperty.PicSizeH = .ReadProperty("PicSizeH", 0)
        m_tButtonProperty.PicSizeW = .ReadProperty("PicSizeW", 0)
        m_tButtonSettings.State = .ReadProperty("State", 0) ' exsNormal
...
'   but only when there are changes in the properties to be updated
    ...
    With PropBag
        ...
        .WriteProperty "PicAlign", m_tButtonProperty.PicAlign, exaLeftOfCaption
        ...
        .WriteProperty "PicSize", m_tButtonProperty.PicSize, 0 ' expNormal
        .WriteProperty "PicSizeH", m_tButtonProperty.PicSizeH, 0
        .WriteProperty "PicSizeW", m_tButtonProperty.PicSizeW, 0
        .WriteProperty "State", m_tButtonSettings.State, 0 ' exsNormal
