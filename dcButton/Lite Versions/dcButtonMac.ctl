VERSION 5.00
Begin VB.UserControl dcButtonMac 
   AutoRedraw      =   -1  'True
   ClientHeight    =   495
   ClientLeft      =   0
   ClientTop       =   0
   ...
   ScaleMode       =   3  'Pixel
   ScaleWidth      =   100
   ToolboxBitmap   =   "dcButtonMac.ctx":0000
End
Attribute VB_Name = "dcButtonMac"
...
Public Enum eMacButtonShapes
    emsCutNone  ' Normal
    emsCutLeft  '
    emsCutRight '
    emsCutSides ' Both left & right
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private emsCutNone, emsCutLeft, emsCutRight, emsCutSides
#End If

Private Enum eMacButtonStates
    emsNormal   ' Normal state
    emsHot      ' Cursor is over the control
    emsDown     ' Mouse/key is down
    emsDisabled ' Disabled state
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private emsNormal, emsHot, emsDown, emsDisabled
#End If

Public Enum eMacPicAlignments
    emaBehindText
    emaBottomEdge
    emaBottomOfCaption
    emaLeftEdge
...
#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private emaBehindText, emaBottomEdge, emaBottomOfCaption
    Private emaLeftEdge, emaLeftOfCaption, emaRightEdge
    Private emaRightOfCaption, emaTopEdge, emaTopOfCaption
#End If

Public Enum eMacPictureSizes
    empNormal ' Use original size of main picture
    emp16x16  ' Small icon size
    emp24x24  ' Standard toolbar icon size
    emp32x32  ' Standard icon size
    emp48x48  ' Explorer thumbnail size
    empCustom ' Use picture size defined in property
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private empNormal, emp16x16, emp24x24, emp32x32, emp48x48, empCustom
#End If

Public Enum eMacUserColors ' User-defined colors
    emcDownColor    ' ---
    emcFocusBorder  ' Available for use with the OverrideColor procedure
    emcGrayColor    ' For advance users only
    emcGrayText     ' Not accessible using VB IDE's properties window
    emcHoverColor   ' Note: Color property usage may vary between button styles
    emcStartColor   ' ---
End Enum

#If False Then
    ' Trick to preserve casing of these variables when used in VB IDE
    Private emcDownColor, emcFocusBorder, emcGrayColor
    Private emcGrayText, emcHoverColor, emcStartColor
#End If
...
Private Type tButtonSettings        ' Runtime/designtime settings
    ...
    State       As eMacButtonStates ' Current drawing state of the button
...
Public Property Get ButtonShape() As eMacButtonShapes
...
Public Property Let ButtonShape(Value As eMacButtonShapes)
...
Public Property Get PictureAlignment() As eMacPicAlignments
...
Public Property Let PictureAlignment(Value As eMacPicAlignments)
...
'   Else the specified value will be set as the main picture instead.
    ...
    If (m_tButtonProperty.PicNormal Is Nothing) Then
        ...
        Exit Property
    Else
        If (m_tButtonProperty.PicSize = empNormal) Then
...
Public Property Get PictureSize() As eMacPictureSizes
...
Public Property Let PictureSize(Value As eMacPictureSizes)
    If (m_tButtonProperty.PicNormal Is Nothing) Then
        m_tButtonProperty.PicSize = empNormal
...
Public Property Let PictureSizeH(Value As Long)
    m_tButtonProperty.PicSize = empCustom   ' If modified then set size to custom
...
'   this procedure is called only when the control has encountered a decisive change.
    ...
    Select Case m_tButtonProperty.PicAlign                          ' Picture on center
        Case emaBehindText                                          ' but behind text
            OffsetRect pr, (bw - pr.Right) \ 2, (bh - pr.Bottom) \ 2
                                                                    '
        Case emaBottomEdge, emaBottomOfCaption                      ' Picture on
            OffsetRect pr, (bw - pr.Right) \ 2, 0                   ' bottom portion
            OffsetRect tr, 0, -tr.Top                               ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = emaBottomEdge) Then    '
            ...
            End If                                                  '
                                                                    '
        Case emaLeftEdge, emaLeftOfCaption                          ' Picture on
            OffsetRect pr, 0, (bh - pr.Bottom) \ 2                  ' left portion
            OffsetRect tr, -tr.Left, 0                              ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = emaLeftEdge) Then      '
            ...
            End If                                                  '
                                                                    '
        Case emaRightEdge, emaRightOfCaption                        ' Picture on
            OffsetRect pr, 0, (bh - pr.Bottom) \ 2                  ' right portion
            OffsetRect tr, -tr.Left, 0                              ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = emaRightEdge) Then     '
            ...
            End If                                                  '
                                                                    '
        Case emaTopEdge, emaTopOfCaption                            ' Picture on
            OffsetRect pr, (bw - pr.Right) \ 2, 0                   ' top portion
            OffsetRect tr, 0, -tr.Top                               ' of caption
                                                                    '
            If (m_tButtonProperty.PicAlign = emaTopEdge) Then       '
...
'   Calculate points needed to draw border of round-rectangular regions.
    ...
    If (m_tButtonProperty.Shape = emsCutLeft) Or _
       (m_tButtonProperty.Shape = emsCutSides) Then
    ...
    If (m_tButtonProperty.Shape = emsCutRight) Or _
       (m_tButtonProperty.Shape = emsCutSides) Then
...
Private Sub DrawButton(Optional State As eMacButtonStates = -1, Optional Force As Boolean)
...
Private Sub DrawButtonCrystalMac(DrawState As eMacButtonStates)
'   Drawing procedure for Crystal.
    ...
    With m_tButtonProperty
        If (.CheckBox And .Value And .Enabled) Then ' Button on checked state
            Select Case DrawState
                Case emsNormal:     DrawState = emsDown
                Case emsHot:        DrawState = emsDown
                Case emsDown:       DrawState = emsDown
            End Select
        ElseIf (DrawState = emsNormal) And (m_bMouseIsDown) Then
                DrawState = emsHot
        End If
    End With
    ...
    With m_tButtonColors
        Select Case DrawState ' Set background color
            Case emsNormal:     Color1 = .BackColor
            Case emsHot:        Color1 = .HoverColor
            Case emsDown:       Color1 = .DownColor
            Case emsDisabled:   Color1 = .GrayColor
        End Select
    ...
    With m_tButtonSettings
        If (.State = emsDisabled) Then ' Disabled state
            ...
            End If
        Else
            If (.State = emsDown) Or (m_tButtonProperty.Value) Then
                If (m_tButtonProperty.Value) And (.State = emsDown) Then
...
'   Draw icon to button with the specified settings.
    ...
    If (m_tButtonProperty.PicSize = emsNormal) Then
    ...
    If (m_tButtonSettings.State = emsDisabled) Then ' For disabled buttons
...
Jmp_DrawImage:
    ...
    If (m_tButtonSettings.State = emsDisabled) Then '
...
'   Draw icon to button with defined special effect being applied.
    ...
    If (m_tButtonSettings.State = emsHot And Not m_tButtonProperty.PicHot Is Nothing) Then
        Set Picture = m_tButtonProperty.PicHot
    ElseIf (m_tButtonSettings.State = emsDown) Or (m_tButtonProperty.Value) Then
        ...
        Case WM_MOUSELEAVE
            ...
            If (Not m_bSpacebarIsDown) Then
                If (m_tButtonProperty.Enabled) Then
                    DrawButton emsNormal, (m_tButtonSettings.Button = 1)
                End If
...
'   Triggered normally when user clicks the control and release it inside the button
    ...
    With m_tButtonProperty                  '
        ...
        If (Not m_bMouseOnButton) Then      ' Check if cursor is over the control
            DrawButton emsNormal, True      ' Redraw is necessary if it is not :)
        Else                                '
            DrawButton emsHot, .CheckBox    ' Force redraw for checkbox mode
        End If                              '
                                            '
        RaiseEvent Click                    '
                                            '
...
'   Only raised when another control of the same window is focused
    ...
    If (m_tButtonProperty.Enabled) Then '
        If (m_bParentActive) Then       ' We need to force redraw
            DrawButton emsNormal, True  ' button on lost of control focus
...
Private Sub UserControl_MouseDown(Button As Integer, Shift As Integer, X As Single, Y As Single)
    ...
    If (Button = 1) Then                    ' vbLeftButton
        ...
        ElseIf (Not m_bMouseOnButton) Then  ' If mouse button is pressed outside
            ...
            DrawButton emsNormal            ' being held down then draw hot state
    ...
    Else
        ...
        If (Not m_bSpacebarIsDown) Then ' Check if spacebar is held down
            ...
            Else                        ' If button should be in down state then
                DrawButton emsHot       ' draw the down state else draw the hot state
            End If                      '
        ElseIf (m_bMouseIsDown) Then    ' Else, check if mouse is held down
            DrawButton emsDown          ' as it moves over the button to
...
'   (On loading of form and before the is control is unloaded to design mode)
    ...
    With PropBag
        ...
        m_tButtonProperty.PicAlign = .ReadProperty("PicAlign", emaLeftOfCaption)
    ...
    Set m_tButtonProperty.PicNormal = .ReadProperty("PicNormal", Nothing)
        ...
        m_tButtonProperty.PicSize = .ReadProperty("PicSize", 0) ' empNormal
        m_tButtonProperty.PicSizeH = .ReadProperty("PicSizeH", 0)
        m_tButtonProperty.PicSizeW = .ReadProperty("PicSizeW", 0)
        m_tButtonSettings.State = .ReadProperty("State", 0) ' emsNormal
...
'   but only when there are changes in the properties to be updated
    ...
    With PropBag
        ...
        .WriteProperty "PicAlign", m_tButtonProperty.PicAlign, emaLeftOfCaption
        ...
        .WriteProperty "PicSize", m_tButtonProperty.PicSize, 0 ' empNormal
        .WriteProperty "PicSizeH", m_tButtonProperty.PicSizeH, 0
        .WriteProperty "PicSizeW", m_tButtonProperty.PicSizeW, 0
        .WriteProperty "State", m_tButtonSettings.State, 0 ' emsNormal
